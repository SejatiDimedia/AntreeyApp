      ...
      <div className="bg-white overflow-x-auto">
        <table className="w-full min-w-[600px]">
          <thead className="text-label-sm text-on-surface-variant/60 uppercase tracking-wider">
            <tr>
              <th className="px-6 py-6 text-left font-semibold whitespace-nowrap">Time</th>
              <th className="px-6 py-6 text-left font-semibold whitespace-nowrap">Customer</th>
              <th className="px-6 py-6 text-left font-semibold whitespace-nowrap">Service</th>
              <th className="px-6 py-6 text-left font-semibold whitespace-nowrap">Staff</th>
              <th className="px-6 py-6 text-left font-semibold whitespace-nowrap">WhatsApp</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-outline-variant/10">
            {bookings.length === 0 ? (
              ...
              <tr key={item.id || index} className={`${item.status === 'confirmed' ? 'bg-primary-container/10 hover:bg-primary-container/20' : 'hover:bg-surface-container-low'} transition-colors group`}>
                <td className="px-6 py-6 font-headline-lg-mobile text-on-surface whitespace-nowrap">{item.timeSlot || '--:--'}</td>
                <td className="px-6 py-6 font-body-lg text-on-surface whitespace-nowrap">{item.customerName || 'Anonymous'}</td>
                <td className="px-6 py-6 text-on-surface-variant whitespace-nowrap">{item.serviceName || 'Service'}</td>
                <td className="px-6 py-6 whitespace-nowrap"><span className="px-3 py-1 bg-surface-container-high rounded-full text-label-md">{item.staffName || 'Any'}</span></td>
